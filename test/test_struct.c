...
void test_struct_block_read(void)
{
	...
	TEST_ASSERT_EQUAL(NUM_BYTES_ADDRESS + NUM_BYTES_CHECKSUM + read_msg.num_bytes + NUM_BYTES_READ_REPLY_OVERHEAD_PLD, motor_tx.len);
...
void test_struct_block_read_write(void)
{
	...
	//create the write message
	misc_read_message_t read_msg = {
		.address = dartt_get_complementary_address(motor_address),
		.index = 0,
	...
	rc = create_struct_write_frame(
		dartt_get_complementary_address(motor_address),
		&controller_config.acceleration,
		sizeof(uint32_t),
		&controller_config,
		&controller_tx
