...
{
    {
        ...
        uint16_t crc = dartt_crc16(array, sizeof(array));
        TEST_ASSERT_EQUAL(0xC6E5, crc);

        // uint16_t crc_lut = dartt_crc16_lut(array, sizeof(array));
...
{
	{
		...
		unsigned int crc32val = dartt_crc32((const unsigned char *)arr, (unsigned int)(sizeof(arr)));
		TEST_ASSERT_EQUAL(0x2C6730D7, crc32val);
	}
	{
		uint32_t testval = 0;
		uint32_t crc32val = dartt_crc32( (unsigned char *)(&testval), sizeof(testval));
		TEST_ASSERT_EQUAL(0x2144DF1C, crc32val);
	}
	{
		unsigned char arr[] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x3f, 0xff, 0xff};
		uint32_t crc32val = dartt_crc32((const unsigned char *)arr, (unsigned int)(sizeof(arr)));
